...
from utils.pan_connector import PanConnector, PANConnector, PanoramaConnector


def test_connector_raises_without_credentials(monkeypatch):
    monkeypatch.delenv("PANOS_USERNAME", raising=False)
    monkeypatch.delenv("PANOS_PASSWORD", raising=False)
    monkeypatch.delenv("PANOS_API_KEY", raising=False)
...
def test_pan_connector_alias_matches():
    """PANConnector should be an alias for PanConnector"""
    assert PANConnector is PanConnector
