...
ansible-vpn: ## vpn config only
	ansible-playbook -i $(INVENTORY) $(ANSIBLE_DIR)/playbooks/site.yml --tags vpn
...
threat-sync: ## sync threat intel to EDL (set FEED_URL + EDL_BUCKET)
	cd $(PYTHON_DIR) && python3 pan_threat_intel_sync.py \
		--feed-url $${FEED_URL} \
		--edl-type $${EDL_TYPE:-ip} \
		--edl-bucket $${EDL_BUCKET:-pa-edl-bucket}
...
lint: ## ansible-lint + terraform fmt check
	...
	terraform -chdir=$(TERRAFORM_DIR) fmt -check -recursive
	cd $(PYTHON_DIR) && python3 -m py_compile *.py utils/*.py && echo "python syntax ok"
